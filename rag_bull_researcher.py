"""
RAG-Enhanced Bull Researcher — cites trading literature to build optimistic cases.
Combines analyst reports with knowledge base retrieval for sophisticated argumentation.
"""
...
def retrieve_relevant_knowledge(query: str, n_results: int = 3,
                                as_of_date: str = None) -> List[dict]:
    """Retrieve relevant knowledge chunks from RAG collections, tagged with chunk IDs.
    ...
    # Point-in-time filter for backtesting (uses int YYYYMMDD for ChromaDB $lte)
    where_filter = None
    if as_of_date:
    ...
    for prefix, collection, n in collections:
        ...
        try:
            ...
            # Fallback without filter if metadata missing on some chunks
...
llm_deep = create_llm("rag_bull_researcher", output_schema=RagBullArgument, max_tokens_override=8000)


@log_agent_call(agent_name="bull_researcher", model_lane="deep")
def generate_rag_bull_case(state) -> dict:
    """Generate bull argument enhanced with RAG knowledge retrieval."""
    tech = state["technical_report"]
    fund = state["fundamentals_report"]
    symbol = state["symbol"]
    
    # Retrieve relevant knowledge based on the signals
    rag_queries = [
        f"bullish {symbol} momentum technical analysis trend following",
        "commodity inventory oversupply mean reversion contrarian trading",
        "technical analysis beats fundamentals short term momentum"
    ]
    
    rag_n = state.get("rag_query_n", 2)
    rag_limit = state.get("rag_chunks_limit", 8)
    as_of_date = state.get("as_of_date")  # For backtesting point-in-time filter

    knowledge_context = []
    for query in rag_queries:
        knowledge_context.extend(retrieve_relevant_knowledge(query, rag_n,
    ...
    prompt = f"""You are a bull researcher with access to trading literature. Build the strongest bull case for {symbol} using both analyst data AND relevant academic/professional knowledge.
...
Build a literature-backed bull case that's stronger than pure speculation."""
    ...
    return {"rag_bull_argument": argument}
...
if __name__ == "__main__":
    ...
    symbol = "USO"
    print(f"=== RAG-Enhanced Bull Case for {symbol} ===\n")
    
    # Get fresh analyst reports
    analyst_result = dual_graph.invoke({"symbol": symbol})
    
    # Generate RAG-enhanced bull case
    result = generate_rag_bull_case({
        "symbol": symbol,
        "technical_report": analyst_result["technical_report"],
        "fundamentals_report": analyst_result["fundamentals_report"]
    })
    
    bull = result["rag_bull_argument"]
    print(f"Strength: {bull.strength} (Conviction: {bull.conviction}/10)")
    print(f"\nThesis: {bull.thesis}")
    
    print(f"\nKey Supports:")
    for i, support in enumerate(bull.key_supports, 1):
        print(f"  {i}. {support}")
    
    print(f"\nLiterature Citations:")
    for i, citation in enumerate(bull.literature_citations, 1):
        print(f"  {i}. [{', '.join(citation.chunk_ids)}] {citation.author}: {citation.claim}")
    
    print(f"\nRebuttals:")
    for i, rebuttal in enumerate(bull.rebuttals, 1):
        print(f"  {i}. {rebuttal}")